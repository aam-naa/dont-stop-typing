...
votes: dict[str, dict[int, dict]] = {}   # room_id -> {voter_role: {"best": role, "worst": role}}

@app.get("/get_room_code/{room_id}")
def get_room_code(room_id: str):
    if room_id not in rooms:
        raise HTTPException(status_code=404, detail="Room not found")
    return {"players": player_code.get(room_id, {})}

@app.post("/vote/{room_id}/{voter_role}")
def submit_vote(room_id: str, voter_role: int, payload: VotePayload):
    if room_id not in rooms:
        raise HTTPException(status_code=404, detail="Room not found")
    votes.setdefault(room_id, {})[voter_role] = {"best": payload.best, "worst": payload.worst}
    return {"status": "recorded"}

@app.get("/results/{room_id}")
def get_results(room_id: str):
    room = rooms.get(room_id)
    if room is None:
        raise HTTPException(status_code=404, detail="Room not found")
    roles = list(room.keys())
    room_votes = votes.get(room_id, {})

    best_counts = {r: 0 for r in roles}
    worst_counts = {r: 0 for r in roles}
    for v in room_votes.values():
        if v["best"] in best_counts:
            best_counts[v["best"]] += 1
        if v["worst"] in worst_counts:
            worst_counts[v["worst"]] += 1

    best_role = max(best_counts, key=best_counts.get) if roles else None
    worst_role = max(worst_counts, key=worst_counts.get) if roles else None
    ...
    for r in roles:
        if r == best_role:
            ...
            points[r] = 500
    return {"points": points, "best": best_role, "worst": worst_role}